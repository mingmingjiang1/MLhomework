...
from calculate_prediction_metrics import *
...
if __name__ == '__main__':
    ...
    parser.add_argument("--indep", help="independent test file")
    parser.add_argument("--format", default='svm')
    ...
    X, y = read_code(args.train, format='%s' % args.format)
    if args.indep:
        ind_X, ind_y = read_code(args.indep, format='%s' % args.format)
    ...
    classes = sorted(list(set(y)))
    save_CV_result_binary(cv_res, '%s_CV.txt' % args.out, para_info)
    mean_auc = plot_roc_cv(cv_res, '%s_ROC_CV.png' % args.out, label_column=0, score_column=2)
    mean_auprc = plot_prc_CV(cv_res, '%s_PRC_CV.png' % args.out, label_column=0, score_column=2)
    cv_metrics = calculate_metrics_cv(cv_res, label_column=0, score_column=2, )
    save_prediction_metrics_cv(cv_metrics, '%s_metrics_CV.csv' % args.out)

    if args.indep:
        save_IND_result_binary(ind_res, '%s_IND.txt' % args.out, para_info)
        ind_auc = plot_roc_ind(ind_res, '%s_ROC_IND.png' % args.out, label_column=0, score_column=2)
        ind_auprc = plot_prc_ind(ind_res, '%s_PRC_IND.png' % args.out, label_column=0, score_column=2)
        ind_metrics = calculate_metrics(ind_res[:, 0], ind_res[:, 2])
        save_prediction_metrics_ind(ind_metrics, '%s_metrics_IND.csv' % args.out)
